...
def run(args):
    ...
    # TODO: adjust folder
    dataset = datasets.CIFAR10('/proj/ciptmp/yx53utuz/datasets', download=True, train=True)
    dataset= CustomDataLoader(dataset)
    ...
    # TODO: adjust folder
    testset = datasets.CIFAR10('/proj/ciptmp/yx53utuz/datasets', download=True, train=False, transform=transform)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        validationsResult.append( (test(model, device, valloader, criterion, set="Validation"),model.state_dict()))
    bestModel= sorted(validationsResult,key=lambda x:x[0])[-1]
    print('Saving best result with'+str(bestModel[0]))
    torch.save(bestModel[1],'best.pth')
...
class CustomDataLoader(Dataset):
    def __init__(self,dataset,crop_prob=0.25) -> None:
        ...
        self.transform = transforms.RandomCrop(size=(16, 16))
        self.transformCompose=transforms.Compose([transforms.ToTensor(),transforms.Resize((32,32)),transforms.Normalize(mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225) ),transforms.RandomHorizontalFlip() ])
